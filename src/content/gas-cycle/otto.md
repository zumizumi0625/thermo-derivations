...
と定義する。等積過程の熱は $\delta q = c_v\,dT$ で計算できる。これは第一法則 $\delta q = du + p\,dv$ で体積一定（$dv=0$）とおくと $\delta q = du = c_v\,dT$ になることに他ならない。断熱過程は同じ第一法則から出る $pv^{\kappa}=\text{const}$ を使う。
...
- **🏗️ 往復機関（力学・燃焼）**: オットーは火花点火レシプロエンジンのモデルで、ピストン－クランク機構が $p$-$v$ 線図の囲む面積 $w_\text{net}=\oint p\,dv$ をトルクへ変換する。圧縮比 $\varepsilon=v_1/v_2$ は行程容積＋すきま容積をすきま容積で割ったエンジン寸法そのもので、燃焼（等積受熱）が力学的な出力に化ける橋渡しになっている。
- **🌀 作動流体の断熱関係**: 効率を決めた $Tv^{\kappa-1}=\text{const}$ は[断熱変化](#/adiabatic-polytropic)の $pv^{\kappa}=\text{const}$ と同じ関係で、流体の**等エントロピー流れ・音速** $a=\sqrt{\kappa R T}$ にもそのまま現れる。$\kappa$ という同じ1つの量がエンジン効率にも音速にも効いている。
