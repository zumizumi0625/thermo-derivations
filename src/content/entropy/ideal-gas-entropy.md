...
出発点はいつもの第一法則 $\delta q=du+p\,dv$。これを $dS=\delta q_{rev}/T$ に通し、理想気体の関係（$du=c_v\,dT$、$pv=RT$）を入れて積分するだけ——過程ごとの条件は一切いらず、状態量の比に落ちる。
...
- **🔥 第一法則**: この公式は $\delta q=du+p\,dv$ を $T$ で割っただけ（$ds=c_v\,dT/T+R\,dv/v$）。**可逆断熱**で $\Delta s=0$ と置くと $c_v\ln(T_2/T_1)=-R\ln(v_2/v_1)$、すなわち $Tv^{\kappa-1}=\text{const}$ ＝[断熱変化](#/adiabatic-polytropic)のポアソンの関係に一致。エントロピー一定が断熱の正体。
- **🌀 流体（等エントロピー流れ）**: $\Delta s=0$（可逆断熱）は圧縮性流れ・ノズル・音速 $a=\sqrt{\kappa RT}$ の標準仮定。実機のノズル/タービンでは $\Delta s>0$ の分だけ理想からずれ、これが断熱効率の落ちとして効く。
- **⚖️ 第二法則（不可逆の定量化）**: 状態量だから不可逆過程でも始終点だけで $\Delta s$ を数えられる。第1項が消える等温では $\Delta s=R\ln(v_2/v_1)$ となり、これがそのまま[自由膨張](#/free-expansion)の $\Delta S>0$ を与える。
